...
def render_shifted_camera(width=256, height=256):
    view_matrix = p.computeViewMatrix(
        cameraEyePosition=[0.84, 0.76, 0.68],
...
for seed in seeds:
    policy = PPO.load(f"models/reach_ppo_noisy_seed{seed}")
    env = make_reach_env()
...
with open(
    "results/reach_camera_shift_randomized_5seed.json",
    "w",
) as file:
...
print(
    "saved: results/reach_camera_shift_randomized_5seed.json"
)